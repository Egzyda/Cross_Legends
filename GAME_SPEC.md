...
```
総価値 = Math.max(物理攻撃, 魔法攻撃) × 1.0  // どちらか高い方だけ
       + (物理防御 × 1.0) + (魔法防御 × 1.0)
       + (速度 × 1.0) + (HP × 0.5) + (MP × 0.7) + (運 × 0.19)

目標: 600 ± 20（各ステータスは5の倍数）
```
...
#### 全体技クールタイム制
- 敵が全体技（`target: 'all_enemies'`）を使用した場合、次のターンの発動率が10%に大幅減衰
- 全体技が10%の確率で連続発動した場合、さらに次のターンも10%継続
- 全体技以外（通常攻撃や単体スキル等）を使用した場合はリセット
- これにより、全体技の理不尽な連発を抑制
...
### 2026-01-29
- **敵AIリアルタイム化**: 敵の行動決定をターン開始時ではなく、各敵の行動順が回ってきた直後に実行するよう変更
- **全体技クールタイム制**: 敵が全体技を使用した場合、次ターンの発動率を10%に減衰。全体技以外を使用でリセット
- **クリティカル合算処理**: 複数のcritBoostバフが付与されている場合、全て合算するよう修正
- 敵AIの高度なルールセクションを追加

### 2026-01-28
- 仕様書を実装データに合わせて全面更新
- ノード構成を更新（ショップノード追加、エリート削減）
- アイテム一覧を詳細化（18種類）
- スキル一覧を実装に合わせて更新
- イベント一覧を15種類に拡充
- キャラクター数を明記（50人）
- 中ボス・ラスボスの具体的なキャラ名を追加
